...
export const COPY = {
  ...
  about: {
    ...
    lines: [
      ...
      { type: 'p', text: '픽셀 하나, 인터랙션 하나까지 진심인 프론트엔드 개발자.' },
      { type: 'p', text: '사용자가 "오..." 하는 순간을 만들 때 가장 즐겁습니다.' },
    ],
  },
  ...
  life: {
    cmd: 'git log --graph --oneline life',
    // [hash, refs, text] — 그래프 모양은 빌더에서
    commits: [
      ['f2026aa', ['HEAD -> now', 'tag: v6.0'], '재미있는 걸 만드는 중 — 웹뷰 진단 도구 · 에이전트 하네스'],
      ['e2025bb', [], 'Flutter 앱 · NestJS — 프론트 너머로 영역 확장'],
      ['d2024cc', [], '실무 투입 — 안전관리 툴 · UI 라이브러리 yoso-ui'],
      ['c2023dd', ['tag: graduation'], '졸업 — 모임 서비스 몽글몽글 · 포트폴리오 v1'],
      ['b2022ee', [], '캠퍼스 프로젝트 러시 — 냉장고를 부탁해 · Darf'],
      ['a2021ff', ['tag: hello-world'], '첫 커밋 — 계정 개설, 잔디의 시작'],
      ['0x2019a', ['tag: init'], '소프트웨어학과 입학'],
    ],
  },
  ...
  tmux: {
    windows: ['0:fetch', '1:stats', '2:life'],
  },
};

// 서브셋 여유분 (카피 수정 대비 흔한 글자 버퍼)
